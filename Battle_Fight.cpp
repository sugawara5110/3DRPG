...
Result Battle::Fight(Hero *hero, Directionkey direction, Result result){
	...
	//敵戦闘不能アクションフラグ
	//LOSTは発生したらずっとなので注意
	for (int i = 0; i < 4; i++)if (enemy[i].Dieflg() == TRUE)e_draw[i].action = LOST;
	...
	if (time_stop_flg == FALSE && E_com_select != NOSELECT && E_com_select != OTHER){
		...
		if (E_com_select == MAG){
			for (int i = 0; i < 4; i++){
				if (enemy[i].Dieflg() == FALSE && e_draw[i].command_run == TRUE){
					MAGprocess<Hero, Enemy>(hero, enemy, &enemy[i], &e_draw[i], e_draw, h_draw, &E_select_obj, &E_MAG_select);
					break;
				}
			}
		}
	...
	//敵回復
	for (int i = 0; i < 4; i++)RCVdraw<Enemy>(&enemy[i], &e_draw[i], E_DMdrawYMAX);
	//プレイヤーダメージ
	...
	if (time_stop_flg == FALSE && com_select != NOSELECT && com_select != OTHER){
		...
		if (com_select == MAG){
			for (int i = 0; i < 4; i++){
				if (hero[i].Dieflg() == FALSE && h_draw[i].command_run == TRUE){
					MAGprocess<Enemy, Hero>(enemy, hero, &hero[i], &h_draw[i], h_draw, e_draw, &select_obj, &MAG_select);
					//コマンド入力完後選択権移動
					SelectPermissionMove(hero);
					break;
				}
	...
	//敵ダメージ
	for (int i = 0; i < 4; i++)DMdraw<Enemy>(&enemy[i], &e_draw[i], E_DMdrawYMAX);
	//エスケープ表示
	...
	//敵全滅→Exp処理
	if (e_draw[0].LOST_fin == TRUE && e_draw[1].LOST_fin == TRUE && e_draw[2].LOST_fin == TRUE && e_draw[3].LOST_fin == TRUE){
		int GetExp = 0;
		for (int i = 0; i < 4; i++){
